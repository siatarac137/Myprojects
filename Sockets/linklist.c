...
#if 0 /* For Performance issue, commenting below code. Adding to list, by
         comapring from tail end. */
  if (list->cmp)
    {
...
/* Return last node's data if it is there.  */
void *
listnode_tail (struct list *list)
{
  struct listnode *node;
...
struct listnode*
listnode_lookup_data (struct list *list, void *data)
{
  struct listnode *node;
  int ret;
